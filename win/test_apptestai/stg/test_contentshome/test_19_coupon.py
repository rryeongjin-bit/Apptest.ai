...
TCID = [ "App_CheckList_431", "App_CheckList_432" ] 
...
def test_005_testrun_info_AOS(main_homepage,aos_flag, sheet):
    ...
    AOS_testrun_coupon= page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_쿠폰받기_쿠폰_다운받기$", re.IGNORECASE) 
    ).first
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_431_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_431_AOS, column="P")
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_431_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_431_iOS, column="R")
