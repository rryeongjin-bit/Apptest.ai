...
# [Prod] Tab bar 프로젝트
# -------------------------------
TCID = ["App_CheckList_003", "App_CheckList_004", "App_CheckList_005",
        "App_CheckList_020", "App_CheckList_022", "App_CheckList_024",
        "App_CheckList_025", "App_CheckList_028", "App_CheckList_029",
        "App_CheckList_030",  "App_CheckList_031"]
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    page = main_homepage
    AOS_testrun_library_common = page.locator(testrun_first).filter(
        has_text=re.compile(r"^내 서재_공통$", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_library_common.wait_for(state="attached", timeout=5000)
        AOS_testrun_library_common.scroll_into_view_if_needed()
        AOS_testrun_library_common.wait_for(state="visible", timeout=5000)
        AOS_testrun_library_common.click()
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_003_AOS= get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_003_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_library_common= page.locator(testrun_first).filter(
        has_text=re.compile(r"^내 서재_공통$", re.IGNORECASE)
    ).first


    try:
        IOS_testrun_library_common.wait_for(state="attached", timeout=5000)
        IOS_testrun_library_common.scroll_into_view_if_needed()
        IOS_testrun_library_common.wait_for(state="visible", timeout=5000)
        IOS_testrun_library_common.click()
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_003_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_003_iOS, column="R")
...
# 비교할 key 값 리스트
keys_to_copy = ["App_CheckList_003", "App_CheckList_004", "App_CheckList_005",
        "App_CheckList_020", "App_CheckList_022", "App_CheckList_024",
        "App_CheckList_025", "App_CheckList_028", "App_CheckList_029",
        "App_CheckList_030",  "App_CheckList_031"]
