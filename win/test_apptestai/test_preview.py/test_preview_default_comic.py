...
# [Prod] 작품홈 미리보기 프로젝트
# -------------------------------
TCID_default_comic = [ "App_CheckList_275", "App_CheckList_276"]
TCID_default_comic_set = [ "App_CheckList_277"]
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    try:
        AOS_testrun_preview_default_comic.wait_for(state="attached", timeout=5000)
        AOS_testrun_preview_default_comic.scroll_into_view_if_needed()
        AOS_testrun_preview_default_comic.wait_for(state="visible", timeout=5000)
        AOS_testrun_preview_default_comic.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result_by_key(sheet, TCID_default_comic, AOS_testrun_info, column="S")

    except Exception as e:
        write_result_by_key(sheet, TCID_default_comic, "No Info", column="S")
        aos_flag["run"] = False
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    if not aos_flag["run"]:
        write_result_by_key(sheet, TCID_default_comic, "N/T", column="P")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_275_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID_default_comic, App_CheckList_275_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_preview_default_comic.wait_for(state="attached", timeout=5000)
        IOS_testrun_preview_default_comic.scroll_into_view_if_needed()
        IOS_testrun_preview_default_comic.wait_for(state="visible", timeout=5000)
        IOS_testrun_preview_default_comic.click()

        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result_by_key(sheet, TCID_default_comic, IOS_testrun_info, column="T")

    except Exception as e:
        write_result_by_key(sheet, TCID_default_comic, "No Info", column="T")
        ios_flag["run"] = False
...
def test_check_testresult_IOS(main_homepage,ios_flag, sheet):
    if not ios_flag["run"]:
        write_result_by_key(sheet, TCID_default_comic, "N/T", column="R")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_275_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet, TCID_default_comic, App_CheckList_275_iOS, column="R")
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    try:
        AOS_testrun_preview_default_comic_set.wait_for(state="attached", timeout=5000)
        AOS_testrun_preview_default_comic_set.scroll_into_view_if_needed()
        AOS_testrun_preview_default_comic_set.wait_for(state="visible", timeout=5000)
        AOS_testrun_preview_default_comic_set.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result_by_key(sheet, TCID_default_comic_set, AOS_testrun_info, column="S")

    except Exception as e:
        write_result_by_key(sheet, TCID_default_comic_set, "No Info", column="S")
        aos_flag["run"] = False
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 테스트 정보 확인 skip")
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    if not aos_flag["run"]:
        write_result_by_key(sheet, TCID_default_comic_set, "N/T", column="P")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_277_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID_default_comic_set, App_CheckList_277_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_preview_default_comic_set.wait_for(state="attached", timeout=5000)
        IOS_testrun_preview_default_comic_set.scroll_into_view_if_needed()
        IOS_testrun_preview_default_comic_set.wait_for(state="visible", timeout=5000)
        IOS_testrun_preview_default_comic_set.click()

        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result_by_key(sheet, TCID_default_comic_set, IOS_testrun_info, column="T")

    except Exception as e:
        write_result_by_key(sheet, TCID_default_comic_set, "No Info", column="T")
        ios_flag["run"] = False
        pytest.skip("⚠️ IOS 테스트 결과 없음 - 테스트 정보 확인 skip")
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    if not ios_flag["run"]:
        write_result_by_key(sheet, TCID_default_comic_set, "N/T", column="R")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_277_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet, TCID_default_comic_set, App_CheckList_277_iOS, column="R")
...
# -------------------------------
# 비교할 key 값 리스트
keys_to_copy = [ "App_CheckList_275", "App_CheckList_276", "App_CheckList_277"]
