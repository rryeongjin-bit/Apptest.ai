...
def test_testrun_info_AOS(main_homepage, write_result,aos_flag):
    ...
    try:
        AOS_testrun_bom_top.wait_for(state="visible", timeout=10000)
        AOS_testrun_bom_top.scroll_into_view_if_needed()
        AOS_testrun_bom_top.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["S350","S351","S352","S353","S354","S355","S356","S357"]:
...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    if not aos_flag["run"]:
        for step in ["P350","P351","P352","P353","P354","P355","P356","P357"]:
    ...
    for step in ["P350","P351","P352","P353","P354","P355","P356","P357"]:
        write_result(step, App_CheckList_334_AOS)
...
def test_testrun_info_IOS(main_homepage,write_result, ios_flag):
    page = main_homepage
    IOS_testrun_bom_top = page.locator(testrun_first).filter(
        has_text = re.compile(r"^\[뷰어\]\s*BOM_상단\s*컨트롤러/본문$", re.IGNORECASE)
    ).first

    try:
        IOS_testrun_bom_top.wait_for(state="visible", timeout=10000)
        IOS_testrun_bom_top.scroll_into_view_if_needed()
        IOS_testrun_bom_top.click()
    
        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["T350","T351","T352","T353","T354","T355","T356","T357"]:
...
def test_check_testresult_IOS(main_homepage, write_result,ios_flag):
    if not ios_flag["run"]:
        for step in ["R350","R351","R352","R353","R354","R355","R356","R357"]:
            write_result(step, "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_334_iOS = get_testrun_status_IOS(page, testrun_status)
   
    for step in ["R350","R351","R352","R353","R354","R355","R356","R357"]:
        write_result(step, App_CheckList_334_iOS)
