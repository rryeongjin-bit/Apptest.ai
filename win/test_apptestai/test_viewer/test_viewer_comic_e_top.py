...
def test_testrun_info_AOS(main_homepage, write_result,aos_flag):
    ...
    AOS_testrun_comic_top = page.locator(testrun_first).filter(
        has_text = re.compile(r"^\[뷰어\]\s*Comic_e북_상단\s*컨트롤러/본문$", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_comic_top.wait_for(state="visible", timeout=10000)
        AOS_testrun_comic_top.scroll_into_view_if_needed()
        AOS_testrun_comic_top.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["S403","S404","S405","S406","S407","S408"]:
...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    if not aos_flag["run"]:
        for step in ["P403","P404","P405","P406","P407","P408"]:
            write_result(step, "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_368_AOS = get_testrun_status_AOS(page, testrun_status)
    
    for step in ["P403","P404","P405","P406","P407","P408"]:
        write_result(step, App_CheckList_368_AOS)
...
def test_testrun_info_IOS(main_homepage,write_result, ios_flag):
    ...
    try:
        IOS_testrun_comic_top.wait_for(state="visible", timeout=10000)
        IOS_testrun_comic_top.scroll_into_view_if_needed()
        IOS_testrun_comic_top.click()
    
        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["T403","T404","T405","T406","T407","T408"]:
...
def test_check_testresult_IOS(main_homepage, write_result,ios_flag):
    if not ios_flag["run"]:
        for step in ["R403","R404","R405","R406","R407","R408"]:
            write_result(step, "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_368_iOS = get_testrun_status_IOS(page, testrun_status)
   
    for step in ["R403","R404","R405","R406","R407","R408"]:
        write_result(step, App_CheckList_368_iOS)
