...
def test_testrun_info_AOS(main_homepage, write_result,aos_flag):
    ...
    try:
        AOS_testrun_pdf_bottom1.wait_for(state="visible", timeout=10000)
        AOS_testrun_pdf_bottom1.scroll_into_view_if_needed()
        AOS_testrun_pdf_bottom1.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["S448","S449","S450","S451","S452"]:
...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    if not aos_flag["run"]:
        for step in ["P448","P449","P450","P451","P452"]:
    ...
    for step in ["P448","P449","P450","P451","P452"]:
        write_result(step, App_CheckList_400_AOS)
...
def test_testrun_info_IOS(main_homepage,write_result, ios_flag):
    ...
    try:
        IOS_testrun_pdf_bottom1.wait_for(state="visible", timeout=10000)
        IOS_testrun_pdf_bottom1.scroll_into_view_if_needed()
        IOS_testrun_pdf_bottom1.click()
    
        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["T448","T449","T450","T451","T452"]:
...
def test_check_testresult_IOS(main_homepage, write_result,ios_flag):
    if not ios_flag["run"]:
        for step in ["R448","R449","R450","R451","R452"]:
            write_result(step, "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")
    ...
    for step in ["R448","R449","R450","R451","R452"]:
        write_result(step, App_CheckList_400_iOS)
