...
# [Prod] 뷰어 프로젝트
# -------------------------------
TCID = [ "App_CheckList_318", "App_CheckList_319", "App_CheckList_320", "App_CheckList_321",
        "App_CheckList_321", "App_CheckList_322", "App_CheckList_323", "App_CheckList_324",
        "App_CheckList_325","App_CheckList_326" ,"App_CheckList_327","App_CheckList_328"]
...
"""
📍 뷰어_Epub_e북, genre-ebook, default-ebook_하단 컨트롤러(상단기능/프로그래스/목차/독서노트/보기설정)
"""    
@pytest.mark.order(4)
@pytest.mark.prod_viewer
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    AOS_testrun_epub_bottom1 = page.locator(testrun_first).filter(
        has_text=re.compile(r"^\[뷰어\]\s*Epub_e북, genre-ebook, default-ebook_하단\s*컨트롤러\(상단기능/프로그래스/목차/독서노트/보기설정\)$", re.IGNORECASE)  
    ).first

    try:
        AOS_testrun_epub_bottom1.wait_for(state="attached", timeout=5000)
        AOS_testrun_epub_bottom1.scroll_into_view_if_needed()
        AOS_testrun_epub_bottom1.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_318_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_318_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_epub_bottom1.wait_for(state="attached", timeout=5000)
        IOS_testrun_epub_bottom1.scroll_into_view_if_needed()
        IOS_testrun_epub_bottom1.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_318_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_318_iOS, column="R")
...
# 자동화 테스트 결과 비교
# -------------------------------
keys_to_copy = [ "App_CheckList_318", "App_CheckList_319", "App_CheckList_320", "App_CheckList_321",
        "App_CheckList_321", "App_CheckList_322", "App_CheckList_323", "App_CheckList_324",
        "App_CheckList_325","App_CheckList_326" ,"App_CheckList_327","App_CheckList_328"]
