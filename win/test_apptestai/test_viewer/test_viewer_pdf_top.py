...
def test_testrun_info_AOS(main_homepage, write_result,aos_flag):
    ...
    try:
        AOS_testrun_pdf_top.wait_for(state="visible", timeout=10000)
        AOS_testrun_pdf_top.scroll_into_view_if_needed()
        AOS_testrun_pdf_top.click()

        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["S441","S443","S445","S446","S447"]:
...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    if not aos_flag["run"]:
        for step in ["P441","P443","P445","P446","P447"]:
    ...
    for step in ["P441","P443","P445","P446","P447"]:
        write_result(step, App_CheckList_393_AOS)
...
def test_testrun_info_IOS(main_homepage,write_result, ios_flag):
    ...
    try:
        IOS_testrun_pdf_top.wait_for(state="visible", timeout=10000)
        IOS_testrun_pdf_top.scroll_into_view_if_needed()
        IOS_testrun_pdf_top.click()
    
        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        for step in ["T442","T443","T444","T445","T446","T447"]:
...
def test_check_testresult_IOS(main_homepage, write_result,ios_flag):
    if not ios_flag["run"]:
        for step in ["R442","R443","R444","R445","R446","R447"]:
            write_result(step, "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_393_iOS = get_testrun_status_IOS(page, testrun_status)
   
    for step in ["R442","R443","R444","R445","R446","R447"]:
        write_result(step, App_CheckList_393_iOS)
