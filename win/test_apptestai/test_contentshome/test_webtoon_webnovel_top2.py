...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    ...
    for step in ["P179","P180"]:
        write_result(step, App_CheckList_162_AOS)
...
def test_check_testresult_IOS(main_homepage, write_result,ios_flag):
    ...
    for step in ["R179","R180"]:
        write_result(step, App_CheckList_162_iOS)
...
"""
📍 작품 홈_webtoon-webnovel_상단 영역_이어보기_대여만료
"""    
...
def test_testrun_info_AOS(main_homepage, write_result,aos_flag):
    page = main_homepage
    AOS_testrun_webtoon_webnovel = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_webtoon-webnovel_상단 영역_이어보기_대여만료_12qa", re.IGNORECASE) 
    ).first
    ...
    try:
        ...
        AOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result("S181", AOS_testrun_info)

    except Exception as e:
        write_result("S181", "No Info")
        aos_flag["run"] = False
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 테스트 정보 확인 skip")
...
def test_check_testresult_AOS(main_homepage, write_result, aos_flag):
    if not aos_flag["run"]:
        write_result("P181", "N/T")
        pytest.skip("⚠️ AOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_173_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result("P181", App_CheckList_173_AOS)
...
def test_testrun_info_IOS(main_homepage, write_result,ios_flag):
    page = main_homepage
    IOS_testrun_webtoon_webnovel = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_webtoon-webnovel_상단 영역_이어보기_대여만료_13qa", re.IGNORECASE) 
    ).first
    ...
    try:
        ...
        IOS_testrun_info = get_testrun_info(page, testrun_id_section)
        write_result("T181", IOS_testrun_info)

    except Exception as e:
        write_result("T181", "No Info")
        ios_flag["run"] = False
        pytest.skip("⚠️ IOS 테스트 결과 없음 - 테스트 정보 확인 skip")
...
def test_check_testresult_IOS(main_homepage, write_result, ios_flag):
    if not ios_flag["run"]:
        write_result("R181", "N/T")
        pytest.skip("⚠️ IOS 테스트 결과 없음 - 결과 확인 skip")

    page = main_homepage
    App_CheckList_173_IOS = get_testrun_status_IOS(page, testrun_status)
    write_result("R181", App_CheckList_173_IOS)
