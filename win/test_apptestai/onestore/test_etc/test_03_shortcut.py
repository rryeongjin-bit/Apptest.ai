...
def test_002_project_shorcut(main_homepage):
    page = main_homepage
    page.click(onestore_shortcut)

    target_project = page.locator(project_title).get_by_text("[완전판] 숏컷")
    try:
        target_project.wait_for(state="visible", timeout=5000)
    except TimeoutError:
        assert False, f"❌ {onestore_shortcut} 폴더 진입 실패"

    assert target_project.is_visible(), f"❌ {onestore_shortcut} 폴더 진입 실패"
