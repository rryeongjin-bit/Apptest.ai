...
TCID1 = [ "App_CheckList_419", "App_CheckList_421"]
TCID2 = [ "App_CheckList_419", "App_CheckList_422"]

def test_001_login_enter_project(main_homepage):
    page = main_homepage
    login_and_select_project(page)

def test_002_project_widget(main_homepage):
    page = main_homepage
    page.click(onestore_widget)

    target_project = page.locator(project_title).get_by_text("[완전판] 위젯")
    try:
        target_project.wait_for(state="visible", timeout=5000)
    except TimeoutError:
        assert False, f"❌ {onestore_widget} 폴더 진입 실패"

    assert target_project.is_visible(), f"❌ {onestore_widget} 폴더 진입 실패"
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet,TCID1, App_CheckList_421_AOS, column="Q")
