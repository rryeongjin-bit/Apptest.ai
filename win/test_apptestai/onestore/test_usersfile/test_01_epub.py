...
def test_006_check_testresult_AOS_epub1(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet, TCID1, App_CheckList_409_AOS, column="Q")
...
📍[사용자파일] ebub_하단 컨트롤러
"""
def test_008_checkresult_AOS_epub2(main_homepage):
    page = main_homepage
    apply_filter_checkbox_AOS(page)

def test_009_testrun_info_AOS_epub2(main_homepage, aos_flag, sheet):
    page = main_homepage
...
def test_010_check_testresult_AOS_epub2(main_homepage, aos_flag, sheet):
    if not aos_flag["run"]:
    ...
    write_result_by_key(sheet, TCID2, App_CheckList_411_AOS, column="Q")

def test_011_back_testrun_list_AOS_epub2(main_homepage, aos_flag):
    back_and_or_reset_AOS(main_homepage, aos_flag.get("run", False))
