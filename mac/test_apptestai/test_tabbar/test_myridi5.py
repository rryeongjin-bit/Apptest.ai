...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    page = main_homepage
    AOS_testrun_myridi5 = page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab bar_마이리디_설정_용량 확인$", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_myridi5.wait_for(state="attached", timeout=5000)
        AOS_testrun_myridi5.scroll_into_view_if_needed()
        AOS_testrun_myridi5.wait_for(state="visible", timeout=5000)
        AOS_testrun_myridi5.click()
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_084_AOS= get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_084_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_myridi5= page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab bar_마이리디_설정_용량 확인$", re.IGNORECASE)
    ).first

    try:
        IOS_testrun_myridi5.wait_for(state="attached", timeout=5000)
        IOS_testrun_myridi5.scroll_into_view_if_needed()
        IOS_testrun_myridi5.wait_for(state="visible", timeout=5000)
        IOS_testrun_myridi5.click()
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_084_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_084_iOS, column="R")
