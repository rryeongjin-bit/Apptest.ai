...
def test_002_project_shorcut(main_homepage):
    page = main_homepage
    page.click(stg_shortcut)

    target_project = page.locator(project_title).get_by_text("[Stage] 숏컷")
    try:
        target_project.wait_for(state="visible", timeout=5000)
    except TimeoutError:
        assert False, f"❌ {stg_shortcut} 폴더 진입 실패"

    assert target_project.is_visible(), f"❌ {stg_shortcut} 폴더 진입 실패"
