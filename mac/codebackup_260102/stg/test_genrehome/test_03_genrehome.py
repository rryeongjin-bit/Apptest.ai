...
TCID = [ "App_CheckList_106", "App_CheckList_109"]
...
def test_005_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    try:
        AOS_testrun_webtoon_daily.wait_for(state="attached", timeout=5000)
        AOS_testrun_webtoon_daily.scroll_into_view_if_needed()
        AOS_testrun_webtoon_daily.wait_for(state="visible", timeout=5000)
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_106_AOS, column="P")
...
def test_009_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_webtoon_daily.wait_for(state="attached", timeout=5000)
        IOS_testrun_webtoon_daily.scroll_into_view_if_needed()
        IOS_testrun_webtoon_daily.wait_for(state="visible", timeout=5000)
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    write_result_by_key(sheet,TCID, App_CheckList_106_iOS, column="R")
