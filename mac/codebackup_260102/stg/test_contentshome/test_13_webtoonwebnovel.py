...
def test_005_testrun_info_AOS(main_homepage,aos_flag, sheet):
    ...
    AOS_testrun_webtoon_webnovel = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_webtoon-webnovel_상단 영역_이어보기_대여만료_AOS$", re.IGNORECASE) 
    ).first
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_173_AOS, column="P")
...
def test_009_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    IOS_testrun_webtoon_webnovel= page.locator(testrun_first).filter(
         has_text=re.compile(r"작품 홈_webtoon-webnovel_상단 영역_이어보기_대여만료_iOS$", re.IGNORECASE) 
    ).first
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    write_result_by_key(sheet,TCID, App_CheckList_173_iOS, column="R")
