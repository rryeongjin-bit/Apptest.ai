...
TCID = [ "App_CheckList_239", "App_CheckList_242", "App_CheckList_243", 
...
def test_005_testrun_info_AOS(main_homepage,aos_flag, sheet):
    ...
    AOS_testrun_default_ebook_top = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_default-ebook_상단 영역$", re.IGNORECASE) 
    ).first
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_239_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_239_AOS, column="P")
...
def test_009_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_default_ebook_top = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_default-ebook_상단 영역$", re.IGNORECASE) 
    ).first

    try:
        IOS_testrun_default_ebook_top.wait_for(state="attached", timeout=5000)
        IOS_testrun_default_ebook_top.scroll_into_view_if_needed()
        IOS_testrun_default_ebook_top.wait_for(state="visible", timeout=5000)
        IOS_testrun_default_ebook_top.click()
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_239_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_239_iOS, column="R")
