...
# [Prod] 뷰어 프로젝트
# -------------------------------
TCID = [ "App_CheckList_383", "App_CheckList_384", "App_CheckList_385", "App_CheckList_386", "App_CheckList_387"]
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    AOS_testrun_webtoon_top = page.locator(testrun_first).filter(
        has_text=re.compile(r"^\[뷰어\]\s*Webtoon_상단\s*컨트롤러/본문$", re.IGNORECASE) 
    ).first

    try:
        AOS_testrun_webtoon_top.wait_for(state="attached", timeout=5000)
        AOS_testrun_webtoon_top.scroll_into_view_if_needed()
        AOS_testrun_webtoon_top.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_383_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_webtoon_top.wait_for(state="attached", timeout=5000)
        IOS_testrun_webtoon_top.scroll_into_view_if_needed()
        IOS_testrun_webtoon_top.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_383_iOS, column="R")
...
# -------------------------------
# 비교할 key 값 리스트
keys_to_copy = [ "App_CheckList_383", "App_CheckList_384", "App_CheckList_385", "App_CheckList_386", "App_CheckList_387"]
