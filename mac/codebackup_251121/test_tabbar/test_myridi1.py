...
# [Prod] Tab bar 프로젝트
# -------------------------------
TCID = ["App_CheckList_074", "App_CheckList_078", "App_CheckList_080",
        "App_CheckList_081", "App_CheckList_082", "App_CheckList_083",
        "App_CheckList_085", "App_CheckList_086", "App_CheckList_087",
        "App_CheckList_090",  "App_CheckList_092"]
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    page = main_homepage
    AOS_testrun_myridi1 = page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab bar_마이리디_내정보관리/리디캐시충전/설정$", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_myridi1.wait_for(state="attached", timeout=5000)
        AOS_testrun_myridi1.scroll_into_view_if_needed()
        AOS_testrun_myridi1.wait_for(state="visible", timeout=5000)
        AOS_testrun_myridi1.click()
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_074_AOS= get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_074_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_myridi1= page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab bar_마이리디_내정보관리/리디캐시충전/설정$", re.IGNORECASE)
    ).first


    try:
        IOS_testrun_myridi1.wait_for(state="attached", timeout=5000)
        IOS_testrun_myridi1.scroll_into_view_if_needed()
        IOS_testrun_myridi1.wait_for(state="visible", timeout=5000)
        IOS_testrun_myridi1.click()
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_074_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_074_iOS, column="R")
...
# 비교할 key 값 리스트
keys_to_copy = ["App_CheckList_074", "App_CheckList_078", "App_CheckList_080",
        "App_CheckList_081", "App_CheckList_082", "App_CheckList_083",
        "App_CheckList_085", "App_CheckList_086", "App_CheckList_087",
        "App_CheckList_090",  "App_CheckList_092"]
