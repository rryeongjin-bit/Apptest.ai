...
"""
📍Tab Bar_검색_SLP_지많찾_장르홈 변경/지많찾 미변경
"""
@pytest.mark.order(4)
@pytest.mark.prod_tabbar
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    page = main_homepage
    AOS_testrun_search3 = page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab Bar_검색_SLP_지많찾_장르홈 변경/지많찾 미변경$", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_search3.wait_for(state="attached", timeout=5000)
        AOS_testrun_search3.scroll_into_view_if_needed()
        AOS_testrun_search3.wait_for(state="visible", timeout=5000)
        AOS_testrun_search3.click()
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_054_AOS= get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_054_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_search3= page.locator(testrun_first).filter(
        has_text=re.compile(r"^Tab Bar_검색_SLP_지많찾_장르홈 변경/지많찾 미변경$", re.IGNORECASE)
    ).first

    try:
        IOS_testrun_search3.wait_for(state="attached", timeout=5000)
        IOS_testrun_search3.scroll_into_view_if_needed()
        IOS_testrun_search3.wait_for(state="visible", timeout=5000)
        IOS_testrun_search3.click()
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_054_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_054_iOS, column="R")
...
# 비교할 key 값 리스트
keys_to_copy = ["App_CheckList_054"]
