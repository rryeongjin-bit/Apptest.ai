...
TCID = [ "App_CheckList_444", "App_CheckList_445", "App_CheckList_447",
        "App_CheckList_448", "App_CheckList_450", "App_CheckList_452",
...
def test_002_project_launchapp(main_homepage):
    page = main_homepage
    page.click(stg_launchapp)

    target_project = page.locator(project_title).get_by_text("[Stage] 앱실행")
    try:
        target_project.wait_for(state="visible", timeout=5000)
    except TimeoutError:
        assert False, f"❌ {stg_launchapp} 폴더 진입 실패"

    assert target_project.is_visible(), f"❌ {stg_launchapp} 폴더 진입 실패"
...
def test_005_testrun_info_AOS(main_homepage,aos_flag, sheet):
    page = main_homepage
    AOS_testrun_onboarding = page.locator(testrun_first).filter(
        has_text=re.compile(r"앱실행_가입온보딩", re.IGNORECASE)
    ).first

    try:
        AOS_testrun_onboarding.wait_for(state="attatched", timeout=5000)
        AOS_testrun_onboarding.scroll_into_view_if_needed()
        AOS_testrun_onboarding.wait_for(state="visible", timeout=5000)
        AOS_testrun_onboarding.click()
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_444_AOS= get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_444_AOS, column="P")
...
def test_009_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_onboarding = page.locator(testrun_first).filter(
        has_text=re.compile(r"앱실행_가입온보딩", re.IGNORECASE)
    ).first

    try:
        IOS_testrun_onboarding.wait_for(state="attached", timeout=5000)
        IOS_testrun_onboarding.scroll_into_view_if_needed()
        IOS_testrun_onboarding.wait_for(state="visible", timeout=5000)
        IOS_testrun_onboarding.click()
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_444_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_444_iOS, column="R")
