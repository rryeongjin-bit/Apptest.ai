...
def test_005_testrun_info_AOS(main_homepage,aos_flag, sheet):
    page = main_homepage
    AOS_testrun_genre_ebook_top = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_genre-ebook_상단 영역_공유$", re.IGNORECASE) 
    ).first

    try:
        AOS_testrun_genre_ebook_top.wait_for(state="attached", timeout=5000)
        AOS_testrun_genre_ebook_top.scroll_into_view_if_needed()
        AOS_testrun_genre_ebook_top.wait_for(state="visible", timeout=5000)
        AOS_testrun_genre_ebook_top.click()
...
def test_006_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_204_AOS = get_testrun_status_AOS(page, testrun_status)
    write_result_by_key(sheet, TCID, App_CheckList_204_AOS, column="P")
...
def test_009_testrun_info_IOS(main_homepage, ios_flag, sheet):
    page = main_homepage
    IOS_testrun_genre_ebook_top = page.locator(testrun_first).filter(
        has_text=re.compile(r"작품 홈_genre-ebook_상단 영역_공유$", re.IGNORECASE) 
    ).first

    try:
        IOS_testrun_genre_ebook_top.wait_for(state="attached", timeout=5000)
        IOS_testrun_genre_ebook_top.scroll_into_view_if_needed()
        IOS_testrun_genre_ebook_top.wait_for(state="visible", timeout=5000)
        IOS_testrun_genre_ebook_top.click()
...
def test_010_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    page = main_homepage
    App_CheckList_204_iOS = get_testrun_status_IOS(page, testrun_status)
    write_result_by_key(sheet,TCID, App_CheckList_204_iOS, column="R")
