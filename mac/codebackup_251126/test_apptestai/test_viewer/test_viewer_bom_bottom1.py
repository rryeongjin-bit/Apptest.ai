...
# [Prod] 뷰어 프로젝트
# -------------------------------
TCID = [ "App_CheckList_342", "App_CheckList_343", "App_CheckList_344","App_CheckList_345", 
           "App_CheckList_346", "App_CheckList_347","App_CheckList_348"]
...
def test_testrun_info_AOS(main_homepage, aos_flag, sheet):
    ...
    try:
        AOS_testrun_bom_bottom1.wait_for(state="attached", timeout=5000)
        AOS_testrun_bom_bottom1.scroll_into_view_if_needed()
        AOS_testrun_bom_bottom1.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_AOS(main_homepage, aos_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_342_AOS, column="P")
...
def test_testrun_info_IOS(main_homepage, ios_flag, sheet):
    ...
    try:
        IOS_testrun_bom_bottom1.wait_for(state="attached", timeout=5000)
        IOS_testrun_bom_bottom1.scroll_into_view_if_needed()
        IOS_testrun_bom_bottom1.wait_for(state="visible", timeout=5000)
...
def test_check_testresult_IOS(main_homepage, ios_flag, sheet):
    ...
    write_result_by_key(sheet, TCID, App_CheckList_342_iOS, column="R")
...
# -------------------------------
# 비교할 key 값 리스트
keys_to_copy = [ "App_CheckList_342", "App_CheckList_343", "App_CheckList_344","App_CheckList_345", 
           "App_CheckList_346", "App_CheckList_347","App_CheckList_348"]
